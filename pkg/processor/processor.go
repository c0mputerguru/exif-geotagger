...
// If scriptWriter is non-nil, it generates a shell script instead of executing exiftool directly.
func TagImages(ctx context.Context, rawDir string, dbPath string, dryRun bool, priorityDevices []string, opts matcher.ProviderOptions, scriptWriter ScriptWriter) error {
	...
	// If script generation is enabled, write the header
	if scriptWriter != nil {
		now := time.Now().UTC().Format(time.RFC3339)
		headerLines := []string{
			"#!/usr/bin/env bash",
			fmt.Sprintf("# Generated by exif-geotagger on %s (UTC)", now),
			fmt.Sprintf("# Config: dry-run=%v, db=%s, search-window=%v, time-threshold=%v, priority-devices=%v",
				dryRun, dbPath, opts.SearchWindow, opts.TimeThreshold, strings.Join(priorityDevices, ",")),
			"# This script tags images with GPS data using exiftool.",
			"# Review and execute manually with: bash script.sh",
		}
		for _, line := range headerLines {
			if err := scriptWriter.WriteLine(line); err != nil {
	...
	// Helper to write a skip comment to the script
	writeSkip := func(path, reason string) error {
		if scriptWriter != nil {
			// Sanitize path for comment (remove newlines)
			...
			comment := fmt.Sprintf("# SKIP: %s - %s", sanitized, reason)
			return scriptWriter.WriteLine(comment)
		}
		return nil
	}
	...
	err = filepath.WalkDir(rawDir, func(path string, d fs.DirEntry, err error) error {
		...
		meta, err := exiftool.ReadMetadata(path)
		if err != nil {
			// Log error for console feedback
			logger.Error("Failed to read metadata for %s: %v", path, err)
			if writeErr := writeSkip(path, fmt.Sprintf("failed to read metadata: %v", err)); writeErr != nil {
				return writeErr
			}
			skipped++
			return nil
		...
		if meta.GPSLatitude != nil && meta.GPSLongitude != nil {
			...
			if writeErr := writeSkip(path, "already has GPS data"); writeErr != nil {
				return writeErr
			}
			skipped++
			return nil
		...
		if err != nil {
			logger.Warn("Warning: No valid timestamp for %s", path)
			if writeErr := writeSkip(path, fmt.Sprintf("no valid timestamp: %v", err)); writeErr != nil {
				return writeErr
			}
			skipped++
			return nil
		...
		if err != nil {
			...
			if writeErr := writeSkip(path, fmt.Sprintf("no matching location: %v", err)); writeErr != nil {
				return writeErr
			}
			skipped++
			return nil
		...
		if scriptWriter != nil {
			// Write a comment with match info (sanitize path for comment)
			sanitized := strings.Map(func(r rune) rune {
				if r == '\n' || r == '\r' {
					return ' '
				}
			...
			timeDiff := match.Timestamp.Sub(ts)
			comment := fmt.Sprintf("# Tag %s using %s (time diff: %v)", sanitized, match.DeviceModel, timeDiff)
			if err := scriptWriter.WriteLine(comment); err != nil {
				return err
			}
			// Build exiftool arguments (includes filePath at the end)
			args := exiftool.BuildExiftoolArgs(path, newMeta)
			// Write the exiftool command
			if err := scriptWriter.WriteCommand("exiftool", args...); err != nil {
				return err
			}
			count++
		} else {
			// Original behavior: actually write metadata
	...
	// If script generation, write footer
	if scriptWriter != nil {
		footer := fmt.Sprintf("# Total: %d files, Tagged: %d, Skipped: %d", count+skipped, count, skipped)
		if err := scriptWriter.WriteLine(footer); err != nil {
			return fmt.Errorf("failed to write script footer: %w", err)
		}
	}

	if dryRun {
		fmt.Printf("Dry run complete. Would have tagged %d images (skipped %d)\n", count, skipped)
	} else if scriptWriter == nil {
		// Only print this summary if not in script mode? Actually if scriptWriter is non-nil, we already printed footer.
		// But we still might want to print to console. Let's print even in script mode for console feedback.
		fmt.Printf("Tagging complete. Successfully tagged %d images (skipped %d)\n", count, skipped)
	} else {
		// In script mode but not dryRun? Still tagging complete? We'll print.
		fmt.Printf("Script generation complete. Tagged: %d, Skipped: %d\n", count, skipped)
	}
	return nil
}
